        ...
        <div class="max-w-4xl mx-auto">
          ...
          <div class="mb-8 font-mono text-sm">
            ...
            <div class="text-orange-500/70 mb-2">
              <span class="text-orange-600">&gt;</span> LOADING WEAPON_EDIT.exe
            </div>
            <div class="text-orange-500 mb-4">
              <span class="text-orange-600">&gt;</span> EDITING: {{ weapon.name }}
            </div>
          ...
          <form @submit.prevent="submitForm" class="space-y-6">
            ...
            <div class="border border-orange-500/30 bg-black/50 p-6 relative">
              <div class="absolute top-0 left-0 w-full h-px bg-orange-500"></div>
              <label class="block font-mono text-xs text-orange-600 mb-2">
                <span class="text-orange-600">&gt;</span> НАЗВАНИЕ*:
              </label>
              <input
                v-model="form.name"
                type="text"
            ...
            <div class="border border-orange-500/30 bg-black/50 p-6 relative">
              <div class="absolute top-0 left-0 w-full h-px bg-orange-500"></div>
              <label class="block font-mono text-xs text-orange-600 mb-2">
                <span class="text-orange-600">&gt;</span> КАТЕГОРИЯ*:
              </label>
              <select
              ...
              <p v-if="errors.category" class="text-red-500 text-sm mt-2">{{ errors.category }}</p>
            </div>
            ...
            <!-- Current Image -->
            <div v-if="weapon.image" class="border border-orange-500/30 bg-black/50 p-6 relative">
              <div class="absolute top-0 left-0 w-full h-px bg-orange-500"></div>
              <label class="block font-mono text-xs text-orange-600 mb-2">
                <span class="text-orange-600">&gt;</span> ТЕКУЩЕЕ ИЗОБРАЖЕНИЕ:
              </label>
              <img :src="`/storage/${weapon.image}`" alt="Current image" class="h-32 object-contain border border-orange-500/20 p-2">
            </div>
            ...
            <div class="border border-orange-500/30 bg-black/50 p-6 relative">
              <div class="absolute top-0 left-0 w-full h-px bg-orange-500"></div>
              <label class="block font-mono text-xs text-orange-600 mb-2">
                <span class="text-orange-600">&gt;</span> НОВОЕ ИЗОБРАЖЕНИЕ:
              </label>
              <input
            ...
            <!-- Current Icon -->
            <div v-if="weapon.icon" class="border border-orange-500/30 bg-black/50 p-6 relative">
              <div class="absolute top-0 left-0 w-full h-px bg-orange-500"></div>
              <label class="block font-mono text-xs text-orange-600 mb-2">
                <span class="text-orange-600">&gt;</span> ТЕКУЩАЯ ИКОНКА:
              </label>
              <img :src="`/storage/${weapon.icon}`" alt="Current icon" class="h-16 object-contain border border-orange-500/20 p-2">
            </div>
            ...
            <div class="border border-orange-500/30 bg-black/50 p-6 relative">
              <div class="absolute top-0 left-0 w-full h-px bg-orange-500"></div>
              <label class="block font-mono text-xs text-orange-600 mb-2">
                <span class="text-orange-600">&gt;</span> НОВАЯ ИКОНКА:
              </label>
              <input
                ...
                class="w-full bg-black border border-orange-500/30 text-white px-4 py-3 font-mono focus:border-orange-500 focus:outline-none"
              />
              <p class="text-gray-500 text-xs mt-2">Оставьте пустым, чтобы сохранить текущую иконку</p>
              <p v-if="errors.icon" class="text-red-500 text-sm mt-2">{{ errors.icon }}</p>
            </div>
...
const handleIconUpload = (e) => {
  form.value.icon = e.target.files[0];
};
...
const submitForm = () => {
  ...
  router.post(`/admin/wiki/weapons/${props.weapon.id}`, formData, {
    onFinish: () => {
