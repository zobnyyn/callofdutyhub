        ...
        <div class="max-w-7xl mx-auto">
          ...
          <div class="mb-8 font-mono text-sm">
            ...
            <div class="text-orange-500/70 mb-2">
              <span class="text-orange-600">&gt;</span> LOADING WEAPON_MANAGER.exe
            </div>
          ...
          <div class="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <Link
              href="/admin/wiki/weapons/create"
          ...
          <div class="border border-orange-500/30 bg-orange-500/5">
              ...
              <table class="w-full font-mono text-sm">
                <thead class="bg-black/50">
                  <tr class="border-b border-orange-500/20">
                    ...
                    <th class="px-4 py-3 text-left text-orange-500">КАТЕГОРИЯ</th>
                ...
                <tbody>
                  ...
                  >
                    <td class="px-4 py-3 text-orange-400">#{{ weapon.id }}</td>
                    <td class="px-4 py-3">
                      <img v-if="weapon.icon" :src="weapon.icon" :alt="weapon.name" class="w-12 h-12 object-contain" />
                      <div v-else class="w-12 h-12 bg-orange-500/10 border border-orange-500/30 flex items-center justify-center">
                        <svg class="w-6 h-6 text-orange-500/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                        </svg>
                      </div>
                    </td>
                    <td class="px-4 py-3 text-white">{{ weapon.name }}</td>
                    <td class="px-4 py-3 text-orange-300 uppercase">{{ weapon.game }}</td>
                    <td class="px-4 py-3 text-orange-300">{{ weapon.category }}</td>
                    <td class="px-4 py-3">
                      <span
                        :class="weapon.is_active ? 'text-green-500' : 'text-red-500'"
                        class="text-xs"
                      >
                    ...
                    <td class="px-4 py-3 text-right">
                      <div class="flex items-center justify-end gap-2">
                        <Link
                          :href="`/admin/wiki/weapons/${weapon.id}/edit`"
                  ...
                  <tr v-if="weapons.data.length === 0">
                    <td colspan="7" class="px-4 py-8 text-center text-orange-500/50">
                      <span class="text-orange-600">&gt;</span> Оружие не найдено
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <!-- Pagination -->
            <div v-if="weapons.data.length > 0" class="p-4 border-t border-orange-500/30 bg-orange-500/10">
              <div class="flex justify-between items-center">
                <div class="text-orange-600 font-mono text-xs">
                  Показано {{ weapons.from }} - {{ weapons.to }} из {{ weapons.total }}
                </div>
                <div class="flex gap-2">
                  <Link
                    v-for="link in weapons.links"
                    :key="link.label"
                    :href="link.url"
                    :class="{
...
const deleteWeapon = (id) => {
  if (confirm('Вы уверены, что хотите удалить это оружие?')) {
    router.delete(`/admin/wiki/weapons/${id}`, {
      preserveScroll: true
    });
  }
};
