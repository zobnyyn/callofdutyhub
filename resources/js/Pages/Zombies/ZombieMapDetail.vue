<template>
  <div class="min-h-screen bg-black text-gray-100 font-[system-ui] relative overflow-hidden">
    ...
    <!-- SEO -->
    <Seo
      :title="zombieMap.name"
      :description="seoDescription"
      :keywords="`${zombieMap.game}, зомби, карта, ${zombieMap.name}`"
      :image="seoImage"
    />
...
const seoDescription = computed(() => {
  const m = props.zombieMap || {};
  if (m.description) return m.description;
  if (m.story) return (m.story || '').slice(0, 160);
  return `${m.name || 'Карта'} — страница с гайдами и информацией по ${m.game || 'Call of Duty'}`;
});

const seoImage = computed(() => {
  const m = props.zombieMap || {};
  if (m.background_image) return `/storage/${m.background_image}`;
